...
CStr Implementation_StrOf(IR* ir,Token* a){
    CStr n = NULL;
...
Token Str_Str_Handler_Ass(IR* ir,Token* op,Vector* args){
    ...
    CStr n2 = Implementation_StrOf(ir,b);
    ...
    if(a->tt==TOKEN_STRING){
        ...
        if(a_var){
            ...
            //Scope_BuildInitVariableRange(ir,a->str,"str",-1,(CStr[]){ CStr_Cpy(n2) });
        }
    }else{
...
Token Str_Str_Handler_Add(IR* ir,Token* op,Vector* args){
    Token* a = (Token*)Vector_Get(args,0);
    Token* b = (Token*)Vector_Get(args,1);

    //printf("ADD: %s + %s\n",a->str,b->str);

    CStr n1 = Implementation_StrOf(ir,a);
    CStr n2 = Implementation_StrOf(ir,b);
    int s1 = CStr_Size(n1);
    int s2 = CStr_Size(n2);
    ...
    TypeMap_PushContained(&ir->types,funcs,
        Type_New("str",8,OperatorInterationMap_Make((OperatorInterater[]){
            ...
            OperatorInterater_Make((CStr[]){ "str",NULL },OperatorDefineMap_Make((OperatorDefiner[]){
                ...
                OperatorDefiner_New(TOKEN_IR_ADD,   (Token(*)(void*,Token*,Vector*))Str_Str_Handler_Add),
